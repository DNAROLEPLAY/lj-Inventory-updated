...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
    ...
    if not itemData then return end
    local itemInfo = QBCore.Shared.Items[itemData.name]
    if itemData.type == "weaponpd" then
        if pdWeapons[itemData.name] then
            if pData.job.type == 'leo' then
                TriggerClientEvent("inventory:client:UseWeapon", src, itemData, itemData.info.quality and itemData.info.quality > 0)
                TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "use")
            else
                QBCore.Functions.Notify(src, 'If you are not Police, Return this Item to a Police Officer IMMEDIATELY', 'error', 5000)
            end
        else
            TriggerClientEvent("inventory:client:UseWeapon", src, itemData, itemData.info.quality and itemData.info.quality > 0)
            TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "use")
    ...
    if itemData.type == "weaponpd" then
        if pdWeapons[itemData.name] then
            if pData.job.type == 'leo' then
                ...
                QBCore.Functions.Notify(src, 'You are not authorized to use this weapon', 'error', 5000)
            end
        else
            TriggerClientEvent("inventory:client:UseWeapon", src, itemData, itemData.info.quality and itemData.info.quality > 0)
            TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "use")
