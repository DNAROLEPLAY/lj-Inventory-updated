...
RegisterNetEvent('inventory:server:SaveInventory', function(type, id)
    if type == "trunk" then
        if Trunks[id] then  -- Check if Trunks[id] is not nil
    ...
    elseif type == "glovebox" then
        if Gloveboxes[id] then  -- Check if Gloveboxes[id] is not nil
            if IsVehicleOwned(id) then
    ...
    elseif type == "stash" then
        if Stashes[id] then  -- Check if Stashes[id] is not nil
            SaveStashItems(id, Stashes[id].items)
        end
    elseif type == "drop" then
        if Drops[id] then  -- Check if Drops[id] is not nil
            Drops[id].isOpen = false
