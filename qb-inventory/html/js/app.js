    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            if (fromData.amount == $toAmount) {
                ...
                } else if ($toSlot == 40 && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        .find("[data-slot=" + $toSlot + "]")
                        .html(
                            '<div class="item-slot-key"><p>5 <i class="fas fa-lock"></i></p></div><div class="item-slot-img"><img src="images/' +
                            newData.image +
                            '" alt="' +
                            newData.name +
                            '" /></div><div class="item-slot-amount"><p>' +
                ...
                if (newData.name.split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ...
                        } else if (newData.info.quality > 25 && newData.info.quality < 60) {
                    ...
                    if (toData.name.split("_")[0] == "weapon") {
                        if (!Inventory.IsWeaponBlocked(toData.name)) {
                            ...
                            if (toData.info.quality < 25) {
                                QualityColor = "rgb(192, 57, 43)";
                            } else if (toData.info.quality > 25 && toData.info.quality < 60) {
                                QualityColor = "rgb(230, 126, 34)";
        ...
        if (!Inventory.IsWeaponBlocked(item.name)) {
            if (item.name.split("_")[0] == "weapon") {
                ...
                if (item.info.quality < 25) {
                    QualityColor = "rgb(192, 57, 43)";
                } else if (item.info.quality > 25 && item.info.quality < 60) {
                    QualityColor = "rgb(230, 126, 34)";
...
Inventory.ToggleHotbar = function(data) {
    if (data.open) {
        ...
        for (i = 1; i <= 5; i++) { // Loop for 5 slots
            var elem =
        ...
        /*
     --   var elem =
--        '<div class="z-hotbar-item-slot" data-zhotbarslot="40"> <div class="z-hotbar-item-slot-key"><p>6 <i style="top: -62px; left: 58px;" class="fas fa-lock"></i></p></div><div class="z-hotbar-item-slot-img"></div><div class="z-hotbar-item-slot-label"><p>&nbsp;</p></div></div>';
  --      $(".z-hotbar-inventory").append(elem);
 --       */
